...
if a == 1:
    b = int(input("您的设备系统类型：1 Windows7及更早   2 Windows8    3 Windows10 及 Windows11   （输入序号回答）"))
    if b == 1:
    ...
    if b == 2:
        er = int(input("您是否有计算机基础：1有   2没有（输入序号回答）"))
        if er == 2:
            print("建议您使用Turbo Top，具体使用方法如下")
            print("请前往http://www.savardsoftware.com/downloads/ttsetup.exe 下载该软件")
            print("按照指示安装，打开")
...
if a == 2:
    d = int(input("您的设备系统类型：1 EMUI   2 MagicUI    3 HarmonyOS   4 其他Android设备    5 IpadOS（输入序号回答）"))
    if d == 1:
        print("建议您使用电脑模式")
        print("下拉屏幕，点击电脑模式")
    ...
    if d == 3:
        ...
        print("在电脑模式中进入教室，尽管使用体验不那么好，但这似乎是目前唯一的办法了")
    if d == 4:
        print("建议使用VMOS Pro虚拟机,在虚拟机进入教室")
        print("注意：该方法来源于网络，未经验证，在验证前务必谨慎使用")
    if d == 5:
        print("课前在设置找到辅助功能，选择触控，打开辅助触控")
        print("点击那个小点，按下“设备”，按“多任务”，选择用“侧拉”打开新的APP")
        print("此时不会自动退出教室")

if a == 3:
    sd = int(input("您的设备系统类型：1 IOS    2 Android    3 HarmonyOS（输入序号回答）"))
    if sd == 2:
        print("建议使用VMOS Pro虚拟机,在虚拟机进入教室")
        print("注意：该方法来源于网络，未经验证，在验证前务必谨慎使用")
    if sd == 3:
